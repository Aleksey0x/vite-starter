...
// Фильтр шрифтов: только критичные начертания (для лендинга достаточно 2-3)
const ALLOWED_WEIGHTS = ['Regular', 'SemiBold', 'Bold', 'Medium'];
const ALLOWED_STYLES = ['normal']; // Italic можно добавить при необходимости
...
// Пути к директориям
const sourceDir = path.resolve(__dirname, '../src/raw/fonts');
const targetDir = path.resolve(__dirname, '../src/assets/fonts');
const fontsScssPath = path.resolve(__dirname, '../src/scss/base/_fonts.scss');
...
function getFontInfo(filename) {
	...
	const fontFamily = lastDashIndex === -1 ? baseName : baseName.slice(0, lastDashIndex);
	const weightName = lastDashIndex === -1 ? '' : baseName.slice(lastDashIndex + 1);

	let fontWeight = 400; // По умолчанию Regular
	let fontStyle = 'normal'; // По умолчанию normal

	if (weightName) {
		const weightLower = weightName.toLowerCase();

		if (weightLower.includes('thin')) fontWeight = 100;
		else if (weightLower.includes('extralight') || weightLower.includes('ultralight'))
			fontWeight = 200;
		else if (weightLower.includes('light')) fontWeight = 300;
		else if (weightLower.includes('regular') || weightLower === 'normal') fontWeight = 400;
		else if (weightLower.includes('medium')) fontWeight = 500;
		else if (weightLower.includes('semibold') || weightLower.includes('demibold'))
			fontWeight = 600;
		else if (weightLower.includes('extrabold') || weightLower.includes('ultrabold'))
			fontWeight = 800;
		else if (weightLower.includes('bold')) fontWeight = 700;
		else if (weightLower.includes('black') || weightLower.includes('heavy')) fontWeight = 900;

		if (weightLower.includes('italic')) fontStyle = 'italic';
	}
...
function convertFonts() {
	...
	for (const ttfPath of ttfFiles) {
		...
		// Фильтруем начертания - оставляем только критичные
		const weightName = fileNameWithoutExt.split('-')[1] || 'Regular';
		if (!ALLOWED_WEIGHTS.includes(weightName)) {
			console.log(`Пропускаем ${fileName} - не входит в список критичных начертаний`);
			continue;
		}
		...
		try {
			...
			// Конвертация только в WOFF2 (WOFF устарел, поддержка 98%+)
			const woff2Result = ttf2woff2(ttfBuffer);
			fs.writeFileSync(path.join(targetDir, `${fileNameWithoutExt}.woff2`), woff2Result);

			console.log(`${fileName} успешно конвертирован в WOFF2`);
		} catch (error) {
...
function generateFontsSCSS(fontsInfo) {
	...
	// Используем относительные пути - Vite перепишет их правильно при сборке с учетом base
	...
	Object.keys(fontFamilies).forEach((family) => {
		...
		scssContent += `$${variableName}: '${family}', sans-serif;\n`;
	});
