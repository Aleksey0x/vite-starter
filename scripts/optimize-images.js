	...
	for (const file of files) {
		...
		for (const format of formats) {
			...
			for (const width of widths) {
				...
				if (format === 'avif') pipeline = pipeline.avif({ quality: quality.avif || 65 }); // качество avif (приоритет)
		...
		// fallback src
		const fallbackFormat = formats.find((f) => f !== 'webp' && f !== 'avif'); // fallback формат
		if (fallbackFormat) {
